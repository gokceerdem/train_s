...
int main()
{
	...
	/*	Mat testing_classifications = Mat(test_sample_number, 1, CV_32FC1);
		...
		for (int ff = 0; ff < test_sample_number; ff++){
			Mat newinstance(1, numberofbins, CV_32FC1);
			for (int v = 0; v < numberofbins; v++){
				newinstance.at<float>(0, v) = testing_data.at<float>( beginn,v);
			}

		//	cout << newinstance << endl;
			float ty = SVM.predict(newinstance,true);
			cout << ty << endl;		
		/*	float confidence = 0;
		int f_to_i = (int)ty;
		confidence = 1.0 / (1.0 + exp(-ty));
		cout << " and its confidence: " << confidence;*/
			newinstance.release();
			beginn++;
		}
...
void hesapla(int image_no, int segment_no, int label_no){
	...
	mystream << imagename1 << std::setfill('0') << std::setw(4) << ivar << imtype;
