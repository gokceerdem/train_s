...
float samples[100][numberofbins] = { 0 };
...
int index = 0 ;
int sindex = 0;
int train_sample_number, test_sample_number, train_sample_number2, test_sample_number2;
...
{	//tstart = time(0);
	...
	if (choice == 'y' || choice == 'Y'){
		...
		do{
			testyap(myData.testArr[sindex][0], myData.testArr[sindex][1]);
			sindex = sindex + 1;
...
void hesapla(int image_no, int segment_no, int label_no){
		...
		//	//cout<<I[myImData.connComp.at(ms).at(0).y][myImData.connComp.at(ms).at(0).x]<<endl;
		//	cout << ms << endl;
		//	waitKey(0);
		//}
...
void testyap(int image_no, int segment_no){
		MatND hist;
		int ivar = image_no;
